        ...
        let par_batches = req_psigs
            ...
            .filter_map(|(issuance_id, shares)| {
                ...
                match bsig {
                    ...
                    Err(e) => {
                        if e != CombineError::TooFewShares {
...
impl Mint {
    ...
    fn combine(
        ...
        if partial_sigs.len() < self.threshold {
            return (Err(CombineError::TooFewShares), MintShareErrors(vec![]));
        }
...
pub enum CombineError {
    #[error("Too few shares to begin the combination")]
    TooFewShares,
    #[error(
...
mod test {
    ...
    fn test_issuance() {
        ...
        assert_eq!(bsig_res, Err(CombineError::TooFewShares));
        assert!(errors.0.is_empty());
